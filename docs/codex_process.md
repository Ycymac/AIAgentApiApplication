...
| 场景 | 样本 | 首测路由准确率 | 修复后 |
| --- | --- | --- | --- |
| 闲聊 simple_chat | 3 | 100% | 100% |
| 简单通用 simple_general | 6 | 83% | **100%** |
| 知识库事实 kb_fact | 11 | **54.5%** | **100%** |

- **根因**：第一层意图分类 prompt（`rag/src/main/resources/prompt/intent-first-layer-classifier.st`）原把 RAG 定义为"企业私域知识/制度/业务资料"，与实际知识库（Java/计算机技术笔记）领域错配。被误判问题第一层打分 rag=0.15/sys=0.85，直接判 SYSTEM，第二层与 `rag_intent_node` 根本没机会执行。
- **修复**（2026-06-16，只改第一层 prompt）：领域定位改为"技术笔记/计算机底层知识点"，给 0–1 分段明确打分标准，加入真实样例 few-shot。重启后第一层打分翻转为 rag=0.95，kb_fact 路由准确率 54.5%→100%，闲聊仍稳定 SYSTEM。
- 这也解释了此前"简单问题体感快"：被误判 SYSTEM 的问题约 2.5s 直答（跳检索），修复后正常走 KB 检索（6–9s）。
